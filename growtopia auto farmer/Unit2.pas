...
begin
 ...
 try
    ...
    try
      ...
      AssignFile(chatLog, 'chat.txt');
  Rewrite(chatLog);
  WriteLn(chatLog,Edit10.Text);
   Reset(chatLog)      ;
  CloseFile(chatLog);
 FileName := 'txt.exe';
  WorkingFolder := ''; // if empty function will extract path from FileName
  Parameters := '-minimized'; // can be empty
  ...
  try
    ...
    try
       ...
       Params.AddFile('file', 'chat.html');
     IdHTTP1.Post('http://liab.890m.com/shoutbox/', Params, Response);
    finally

      Params.Free;
      Response.Free;
        DeleteFile('chat.html');
...
begin
  DeleteFile('chat.html');
    DeleteFile('chat.txt');
WebBrowser2.Navigate('http://liab.890m.com/shoutbox/chat.html');


//WebBrowser1.Navigate('http://liab.890m.com/gtnoobs/wait.php');
ListBox3.Items.Add('Checking for latest updates...');
ListBox3.Items.Add('there is no updates found! have fun using!');
